...
\subsection*{Integration of Non-negative functions} 
    ...
    \begin{tbox}{\textbf{Fatou's Lemma:} Let $0 \leq f_n$. Then 
        \[\int \liminf f_n \; d\mu \leq \lim_{n \to \infty} \int f_n\; d\mu\]}
...
\section{Oct 24}
    ...
    \subsection*{Modes of Convergence}
        \begin{tbox}{\textbf{Egnoff Theorem:} Suppose $\mu(X) < \infty$ and $f_n \to f$ a.e. Then $\forall \ep > 0$, $\exists E \sub X$ such that $\mu(E) < \infty$ with $f_n - f \to 0$ uniformly on $E^c$}
            \emph{Proof:} 

            Suppose $f_n \to f$ everywhere. 

            Recall the set of no convergence: $\phi = \{x: \exists \delta_x > 0 \st \forall N, \exists n \geq N \; \abs{f_n(x) - f(x)} \geq \delta_x\}$
